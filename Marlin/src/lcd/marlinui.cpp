...
bool pause_action_flag = 0;   //暂停后回零动作标志位
int temphot= 0;
int tempbed = 0;
...
void MarlinUI::update() {
  ...
  if(pause_action_flag && printingIsPaused() && !planner.has_blocks_queued())
  {
    ...
    queue.enqueue_now_P(PSTR("G1 X0 Y0 F1200"));
  }
  ...
  void MarlinUI::pause_print() {
    ...
    if(((thermalManager.temp_bed.target - 3) > thermalManager.temp_bed.celsius) || ((thermalManager.temp_hotend[0].target - 3) > thermalManager.temp_hotend[0].celsius))
    {
      pause_action_flag = false;
    }
    else
  ...
  void MarlinUI::resume_print() {
    ...
    queue.enqueue_one_now(PSTR("M420 S0")); //Enable automatic compensation function rock_2021.07.17
    ...
    // queue.enqueue_one_now(PSTR("G92.9E0")); // 	Reset E to 0
    // queue.enqueue_one_now(PSTR("G0 E5 F1000")); 

    // char cmd1[20], str_1[16];
    // // Restore E position with G92.9
    // sprintf_P(cmd, PSTR("G92.9E%s F1000"), dtostrf(current_position.e, 1, 3, str_1));
    // queue.enqueue_one_now(cmd1);
