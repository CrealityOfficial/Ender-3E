...
G29_TYPE GcodeSuite::G29() {
    ...
    abl.measured_z = 0;
      ...
      for (PR_OUTER_VAR = 0 ; PR_OUTER_VAR < PR_OUTER_SIZE && !isnan(abl.measured_z); PR_OUTER_VAR++) {
        ...
        for (PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; pt_index++, PR_INNER_VAR += inInc) {
          ...
          if (abl.verbose_level) SERIAL_ECHOLNPAIR("Probing mesh point ", pt_index, "/", abl.abl_points, "."); //pt_index 目前的调平点，abl.abl_points总的调平点数
          ...
          if (isnan(abl.measured_z)) {
            ...
            TERN_(EXTENSIBLE_UI, ExtUI::onMeshUpdate(abl.meshCount, z));
              ...
              if(!IS_SD_PRINTING())
              {
                ...
                //   change_page_font = 26;
                // }
                  rtscheck.RTS_SndData((uint16_t)((6.25 * pt_index) / 2) , AUTO_BED_LEVEL_TITLE_VP);  //6.25为 1/16 特为16点调平计算的公式，后期需要修改算法
  ...
  process_subcommands_now_P(PSTR("G0 Z0 F600"));   //G29后直接回到Z0位置方便调节Z轴偏移
